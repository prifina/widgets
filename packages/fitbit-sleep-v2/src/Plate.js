  ...
  switch(selectedOption) {
    ...
    case "Year":
      if (widgetsData.year.dates.length > 0){
        ...
        console.log(widgetsData.year)
        console.log(widgetsData)

        widgetsData.year.processedData.forEach(function(day) {
          var month = day.starttime.split("-")[1];
          if (month in dataByMonth) {
            console.log(typeof day.deep.minutes)
            dataByMonth[month].deep += Number(day.deep.minutes);
            dataByMonth[month].timeinbed += Number(day.timeinbed);
            dataByMonth[month].light += Number(day.light.minutes);
            dataByMonth[month].awake += Number(day.awake.minutes);
            dataByMonth[month].rem += Number(day.rem.minutes);
            dataByMonth[month].days++;
          } else {
            dataByMonth[month] = {
              awake: Number(day.awake.minutes),
              deep: Number(day.deep.minutes),
              light: Number(day.light.minutes),
              rem: Number(day.rem.minutes),
              timeinbed: Number(day.timeinbed),
              days: 1
            };
          }
        });
        ...
        for (let month in dataByMonth) {
          ...
          dataByMonth[month].timeinbed = dataByMonth[month].timeinbed / dataByMonth[month].days;
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(awake) => {
                return Math.floor((awake / 60) * 100) / 100;
              }}
            />

            <Bar barSize={27} stackId="a" name="awake" dataKey={selectedOption === "Year" ? "awake" :"awake.minutes"} fill="#C8FFF5" />
            <Bar barSize={27} stackId="a" name="light" dataKey={selectedOption === "Year" ? "light" :"light.minutes"} fill="#54B4A3" />
            <Bar barSize={27} stackId="a" name="deep" dataKey={selectedOption === "Year" ? "deep" :"deep.minutes"} fill="#269582" />
            <Bar barSize={27} stackId="a" name="ren" dataKey={selectedOption === "Year" ? "rem" :"rem.minutes"} fill="#236B5F" />
          </BarChart>
        </ResponsiveContainer>
      </Box>
    </Box>
